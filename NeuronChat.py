import os
os.system("cls")
print("Thanks for you choice NeuronAI (≧∇≦)/~")
print(" /\\_/\\")
print("( o.o )     NeuronChat")
print(">  @  <") 
...
class SimpleChatbot:
    def __init__(self, model_path='best_model.pth', vocab_path='vocab.pkl'):
        ...
        with Progress(transient=True) as progress:
            ...
            self.model = Seq2SeqWithAttention(
                ...
                DecoderWithAttention(256, len(self.vocab), num_layers, 128)
            ).to(self.device)
            
            # 安全加载模型参数
            self.model.load_state_dict(
                # 遇到报错删掉 weights_only 即可
    ...
    def generate_response(self, input_text: str) -> Optional[str]:
        try:
            ...
            # 生成响应
            response = []
            with console.status("[bold green]Wait a moment...", spinner="dots"):
                with torch.no_grad():
            ...
            print(Panel(error_text, title="错误"))
            return None
    ...
    def chat(self):
        ...
        while True:
            try:
                    ...
                    if cmd[0] == 'temp' and len(cmd) > 1:
                        try:
                            ...
                            if 0.1 <= new_temp <= 2.0:
                                ...
                                console.print("[red]温度值须在0.1到2.0之间[/]")
                        except ValueError:
                        ...
                        if len(cmd) == 2 and cmd[1] == 'off':
                            ...
                            print("What the fuck do you want to say?")
                    
                    elif cmd[0] == 'cls':
                        os.system("cls")
                    
                    else:
                        console.print("[red]bad command")
                    continue
                ...
                #console.print("\n[red bold]⚠ 安全中断已触发[/]")
                """
                防止有人误触，所以这个按下 `ctrl + C` 程序中断的功能，被我砍掉了
                绝不是因为某人自己误触了
                """
                pass
...
if __name__ == '__main__':
    ...
    console.print(Panel(
        f"欢迎使用 {ver}\n"
        "Visit https://xinhaitianze.github.io/Neuron.html for more information.",
        subtitle="输入 /help 以查看控制命令"
    ))
